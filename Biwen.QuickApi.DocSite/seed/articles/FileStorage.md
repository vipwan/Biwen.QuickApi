...
Biwen.QuickApi提供本地物理文件存储和阿里云OSS文件存储两种方式。
...
```csharp
service.AddKeyedLocalFileStorage(key, path);
```

### 阿里云OSS文件存储

添加引用 Biwen.QuickApi.Storage.AliyunOss

```csharp
services.AddKeyedAliyunOssStorage(key,connectionString);
```

### 参考测试用例

你可以参考一下的测试用例使用文件存储服务
